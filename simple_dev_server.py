#!/usr/bin/env python3
"""
Simple development server that watches YAML files and regenerates JavaScript
...
class YAMLProcessor:
    ...
    def simple_yaml_parse(self, content):
        """Simple YAML parser for basic structures - no external deps needed"""
        result = {}
        current_dict = result
        dict_stack = [result]
        indent_stack = [0]
            ...
            if stripped.startswith('- '):
                ...
                if ':' in item_content:
                    ...
                    if value:
                        ...
                        item_dict['value'] = self.parse_value(value)
                    
                    dict_stack.append(item_dict)
                    indent_stack.append(indent)
                else:
                    # Simple list item
                    if 'items' not in current_dict:
                        current_dict['items'] = []
                    current_dict['items'].append(self.parse_value(item_content))
    ...
    def load_data(self):
        """Load all YAML data files using simple parser"""
        data_dir = '_data'
    ...
    def generate_calculator_js(self):
        ...
        # Extract benchmarks for JavaScript
        benchmarks = {}
        if 'benchmarks' in self.data and 'benchmarks' in self.data['benchmarks']:
            benchmarks = self.data['benchmarks']['benchmarks']
        
        js_content = f'''
// Auto-generated Security Program Calculator from YAML data
document.addEventListener('DOMContentLoaded', function () {{
    // Embedded benchmark data from YAML files
    const benchmarks = {json.dumps(benchmarks, indent=4)};
